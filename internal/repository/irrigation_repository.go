...
type IrrigationRepository interface {
    DailyAggregation(ctx context.Context, farmID uint, start, end time.Time, sectorID *uint) ([]DailyRow, error)
...
func (r *irrigationRepository) DailyAggregation(ctx context.Context, farmID uint, start, end time.Time, sectorID *uint) ([]DailyRow, error) {
    var rows []DailyRow
    q := r.db.WithContext(ctx).Table("irrigation_data").
        Select("date_trunc('day', start_time) as day, SUM(nominal_amount) as nominal_amount_mm, SUM(real_amount) as real_amount_mm, AVG(efficiency) as average_efficiency, COUNT(*) as event_count").
